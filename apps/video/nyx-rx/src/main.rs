...
fn main() -> eframe::Result {
    ...
    let channel_addr = cli_arg("--channel", &format!("127.0.0.1:{DEFAULT_RX_PORT}"));
    ...
    let shared = Arc::new(Shared {
        ...
        udp_mode: AtomicBool::new(!cli_arg("--udp", "").is_empty()),
        ...
        tlm_out: Mutex::new({
            let a = cli_arg("--tlm-out", "127.0.0.1:14558");
    ...
    spawn_control(shared.clone(), cli_arg("--ctl", "127.0.0.1:7203"));
    ...
    {
        let addr = cli_arg("--tlm-in", "0.0.0.0:14555");
        if addr != "0" {
    ...
    if let Ok(p) = cli_arg("--web", "7280").parse::<u16>() {
        if p != 0 {
